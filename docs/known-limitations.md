...
- 商品 API 的 `image_url`、`image_urls` 和 `location` 可为空或为空数组。详情图库只采集公开图片且最多九张；当前真实详情页的图库使用不带 `main` 容器的 `fadeInImg` CSS Modules 类，选择器同时保留旧结构兼容。页面结构异常、登录失效或风控会安全停止或记录该商品错误，商城必须提供缺图和无地区降级，API 不会伪造数据。
